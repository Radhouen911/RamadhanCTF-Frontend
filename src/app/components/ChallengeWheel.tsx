        ...
        >
          ...
          >
              ...
              {categories.map((cat, i) => {
                ...
                return (
                  ...
                  >
                    <stop
                      offset="0%"
                      stopColor={cat.darkColor}
                      stopOpacity="0.95"
                    />
                    <stop
                      offset="60%"
                      stopColor={cat.midColor}
                      stopOpacity="0.75"
                    />
                    <stop
                      offset="100%"
            ...
            <circle
              ...
              r={278}
              fill="none"
              stroke="rgba(212,165,32,0.35)"
              strokeWidth="3"
              filter="url(#rim-glow-f)"
            />
            <circle
              cx={CX}
              cy={CY}
            ...
            {Array.from({ length: 24 }, (_, i) => {
              ...
              return (
                <line
                  ...
                  y2={outer.y}
                  stroke={
                    isMajor ? "rgba(212,165,32,0.7)" : "rgba(212,165,32,0.3)"
                  }
            ...
            {categories.map((_, i) => {
              ...
              return (
                <circle
                  ...
                  cy={pt.y}
                  r={3}
                  fill="rgba(212,165,32,0.7)"
                  stroke="rgba(212,165,32,0.3)"
                  strokeWidth="1"
                />
              );
            })}
            ...
            {categories.map((cat, i) => {
              const pt = polarToCartesian(
                278,
                i * segAngle + segAngle / 2 + ROTATION_OFFSET,
              );
              const isActive =
                selectedCategory === cat.id || hovered === cat.id;
              return (
                <polygon
                  key={cat.id}
            ...
            {!selectedCategory && (
              <g fill="#fbbf24">
                <polygon
                  points={`${CX + 26},${CY - 32} ${CX + 28.5},${CY - 39} ${CX + 31},${CY - 32} ${CX + 28.5},${CY - 25}`}
                  opacity="0.95"
                />
            ...
            {selectedCategory && selectedCat && (
              <g>
                <text
                  ...
                  letterSpacing="3"
                  style={{
                    filter: `drop-shadow(0 0 8px ${selectedCat.color})`,
                  }}
                >
                ...
                <text
                  ...
                  letterSpacing="1.5"
                >
                  {solvedCounts[selectedCategory]?.solved ?? 0}/
                  {solvedCounts[selectedCategory]?.total ?? 0} SOLVED
                </text>
              </g>
            )}
                ...
                >
                  <IconComp
                    ...
                    color={
                      isActive
                        ? cat.color
                        : `rgba(255,255,255,${isDimmed ? 0.3 : 0.8})`
                    }
                    style={{
                      transition:
                        "color 0.3s ease, filter 0.3s ease, transform 0.3s ease",
                      filter: isActive
                        ? `drop-shadow(0 0 10px ${cat.color}) drop-shadow(0 0 16px ${cat.color}80)`
                        : "none",
                ...
                >
                  <span
                    style={{
                      ...
                      color: isActive
                        ? cat.color
                        : `rgba(255,255,255,${isDimmed ? 0.25 : 0.65})`,
                      transition:
                        "color 0.3s ease, font-size 0.3s ease, text-shadow 0.3s ease",
                      display: "block",
                      whiteSpace: "nowrap",
                      textShadow: isActive
                        ? `0 0 10px ${cat.color}, 0 0 18px ${cat.color}88`
                        : "none",
                    }}
                  >
                    {cat.name}
