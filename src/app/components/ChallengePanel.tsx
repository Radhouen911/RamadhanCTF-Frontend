...
import { ChevronDown, Flag, Users, Star, CircleCheck, Circle, Lightbulb, X, ExternalLink, Globe, Lock, Terminal, Search, Eye, Sparkles, Copy } from 'lucide-react';
...
const difficultyConfig = {
  Easy:   { color: '#34d399', bg: 'rgba(52,211,153,0.12)',  border: 'rgba(52,211,153,0.25)'  },
...
function ChallengeCard({
  ...
  const [containerStatus, setContainerStatus] = useState<'idle' | 'spawning' | 'active'>('idle');
  const [containerInfo, setContainerInfo] = useState<{ ip: string; port: number } | null>(null);

  const handleSpawnContainer = () => {
    setContainerStatus('spawning');
    setTimeout(() => {
      setContainerStatus('active');
      setContainerInfo({ ip: '10.10.14.23', port: Math.floor(Math.random() * 1000) + 3000 });
    }, 2000);
  };
    ...
    >
      ...
      >
        <div className="flex-shrink-0">
          {isSolved
            ? <CircleCheck size={16} color="#34d399" style={{ filter: 'drop-shadow(0 0 4px #34d399)' }} />
        ...
        <div className="flex-1 min-w-0">
          <p style={{
            ...
            fontSize: '14px',
            fontWeight: '600',
            color: isSolved ? 'rgba(255,255,255,0.5)' : 'rgba(255,255,255,0.9)',
            letterSpacing: '0.5px',
        ...
        <div className="px-4 py-3">
          ...
          <div className="flex items-center gap-4 mb-3">
            <div className="flex items-center gap-1.5">
              ...
              <span style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: '12px', color: 'rgba(255,255,255,0.4)' }}>
          ...
          {challenge.hint && (
            <div className="flex items-start gap-2 p-2.5 rounded-lg mb-3" style={{
              background: 'rgba(251,191,36,0.06)',
              border: '1px solid rgba(251,191,36,0.15)',
            }}>
              <Lightbulb size={13} color="rgba(251,191,36,0.7)" className="flex-shrink-0 mt-0.5" />
              <p style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: '12px', color: 'rgba(251,191,36,0.65)', lineHeight: '1.5' }}>
                {challenge.hint}
              </p>
            </div>
          )}
            ...
            <div className="mb-4">
              ...
              {containerStatus === 'spawning' && (
                <div className="flex items-center gap-3 px-3 py-2 rounded-lg bg-white/5 border border-white/10">
                  <div className="w-4 h-4 border-2 rounded-full animate-spin" style={{ borderColor: categoryColor, borderTopColor: 'transparent' }} />
                ...
                <div className="p-3 rounded-lg bg-[#0a0f1c] border border-white/10 relative group">
                   <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-green-400 font-[Rajdhani] uppercase tracking-wider font-bold">Instance Active</span>
                      </div>
                      <span className="text-[10px] text-white/30 font-[Rajdhani]">Expires in 59:20</span>
                   </div>
                   <div className="flex items-center gap-3 font-mono text-xs text-white/80 bg-black/40 p-2 rounded border border-white/5">
                      <span className="select-all">nc {containerInfo.ip} {containerInfo.port}</span>
                      <button 
                        className="ml-auto text-white/40 hover:text-white transition-colors"
                        onClick={() => navigator.clipboard.writeText(`nc ${containerInfo.ip} ${containerInfo.port}`)}
                      >
                        <Copy size={12} />
                      </button>
                   </div>
              ...
              {showFlag ? (
                <div className="flex gap-2">
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') { onSolve(); setShowFlag(false); setFlagInput(''); }
                    }}
                    style={{
                      flex: 1,
                      background: 'rgba(255,255,255,0.05)',
                  ...
                  />
                  <button
                    onClick={() => { onSolve(); setShowFlag(false); setFlagInput(''); }}
                    style={{
...
export function ChallengePanel({ category, solvedIds, onSolve, onClose }: ChallengePanelProps) {
  const IconComp = iconMap[category.iconName];
  const totalPts = category.challenges.reduce((s, c) => s + c.points, 0);
  const earnedPts = category.challenges.filter((c) => solvedIds.has(c.id)).reduce((s, c) => s + c.points, 0);
  const solvedCount = category.challenges.filter((c) => solvedIds.has(c.id)).length;
    ...
    >
      ...
      <div style={{
        ...
        <div className="flex gap-3 mb-4">
          {[
            { label: 'Solved', value: `${solvedCount}/${category.challenges.length}` },
            { label: 'Points', value: `${earnedPts}/${totalPts}` },
          ].map((stat) => (
            <div key={stat.label} className="flex-1 px-3 py-2 rounded-lg" style={{
              background: 'rgba(255,255,255,0.03)',
              border: '1px solid rgba(255,255,255,0.06)',
            }}>
              <p style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: '18px', fontWeight: '700', color: category.color, lineHeight: 1 }}>
                {stat.value}
              </p>
              <p style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: '10px', letterSpacing: '1.5px', textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)', marginTop: '3px' }}>
                {stat.label}
              </p>
            </div>
          ))}
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <span style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: '10px', letterSpacing: '2px', textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)' }}>
              Progress
            </span>
            <span style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: '11px', fontWeight: '600', color: category.color }}>
